...
var flashUpdate = function() {
  ...
  flashUpdate.vars = {
    ...
    mobile: 'You are using a mobile device without flash.<br>' + 
            'Click <a href="/m/">here</a> to go to iCarly.com mobile.' 
  }
  ...
  if (flash_version < 9)
  {
    ...
    if (os === 'mac') {   
      if (os_version >= 10.4)     
        install_url = '/downloads/flash-install-mac-4-5.dmg'      
      else if (os_version >= 10.2) 
        install_url = '/downloads/flash-install-mac-1-3.dmg'
                    
    } else if (os === 'windows') { 
      if (window.ActiveXObject) {
        if (os_version >= 5.0)
          install_url = '/downloads/flash-install-win-ie-5-7.exe'
        else if ((os_version > 0) && (flash_version < 9))
          install_url = '/downloads/flash-install-win-ie-1-4.exe'
      } else {
        if (os_version >= 5.0)
          install_url = '/downloads/flash-install-win-standard-5-7.exe'
        else if (os_version > 0)
          install_url = '/downloads/flash-install-win-standard-1-4.exe'
      }
