// change swf embed markup

var currVid = {vid: -1, cat: -1, chan: -1},
    loadingVid = {vid: -1, cat: -1, chan: -1},
    exit_archive_flag = false,
    is_archive_flag = false, 
    video_comment_url = '/iVideo/index.html?comment=sent#vid',
    discreteVid = false;
    //video_url_chan, video_url_cat;
        ...
        if ((curLiChildren_arr[ii].nodeType == 1) && (curLiChildren_arr[ii].nodeName == 'A')) {
          ...
          if (searchObject.substring(0, searchLen) == searchString) {
            ...
            if (chanId == curTabId) {
              ...
              $(curLiChildren_arr[ii]).removeClass('selected');
            }
          }
        }
      }
...
function setCategoryPicker(chanId) {
    ...
    if (list_jquery.length > 0) {
      ...
      list_jquery[0].onchange = function () {
        ...
        pickCategory(category, channel);        
      }
    }     
}

function pickCategory(category, channel) {
...
function refreshPlayer(playerConfig) {
  ...
  // swap config on ad-free video page
  if ( noAds() && !playerConfig.match( /ad-free/ ) ) {
    playerConfig = playerConfig.replace( 'videoxml/', 'videoxml/ad-free/' )
  }
  ...
  flashVars.playerConfig = discreteVid ? getDiscreteConfig() : playerConfig;
  
  if ( noAds() ) {
      flashVars.playerConfig = adFreeConfig( flashVars.playerConfig );
  }
  ...
  swfobject.embedSWF(swf_url, 'player', '504',  "332", "9.0.0", '', flashVars, {'allowFullScreen':'true', 'allowScriptAccess':'sameDomain'}) 
}
  ...
  if ( typeof(ALL_CHANNELS[chan]) != 'undefined')
    if ( typeof(ALL_CHANNELS[chan].categories[cat]) != 'undefined')
      if ( typeof(ALL_CHANNELS[chan].categories[cat].videoxml) != 'undefined') {
        ...
        $.extend(currVid, {cat:cat,chan:chan});
...
// JMS: get vid id from /iVideo/clip_xxx if present.  This will override hash.
function getVidObjFromHash(obj) {
  // get video id, channel, and category from hash
  var hash = getHash();
  //alert('getVidObjFromHash: hash: '+hash)

  var vid_id = document.location.href.match(/.*?video_(\d*)\D*/);
  if (vid_id) {
    discreteVid = true;
    obj.vid = vid_id[1];
  } else {
  ...
  var chan_id = document.location.href.match(/.*?chan_(\d*)\D*/);
  if (chan_id) {
    obj.chan = chan_id[1];
  //video_url_chan = obj.chan;
  } else {
  ...
  var cat_id = document.location.href.match(/.*?cat_(\d*)\D*/);
  if (cat_id) {
    obj.cat = cat_id[1];
    //video_url_cat = obj.cat;
  } else {
...
function drawVideoListElement(vid_obj, lokr_flag) {
  ...
  // JMS
  //alert("chanId="+vid_obj.chanId+" and catId="+vid_obj.catId);
  var slug = video_dictionary[vid_obj.id]['title'].toLowerCase().replace(/[^a-z0-9]/g,'-').replace(/-+/g,'-').replace(/-$/g,'');
  
  html_str += "\n<a href='/iVideo/video_"+vid_obj.id+"-" + slug;
  if (vid_obj.chanId != -1 ) html_str += "/chan_" +vid_obj.chanId;
  if (vid_obj.catId != -1) html_str += "/cat_" + vid_obj.catId;
...
function drawCategoryList(chanId) {
  ...
  var chanName = ALL_CHANNELS[chanId].name,
      html_str = '<div class="category_select">',
      catName,
      firstCat = ' class="selected"',
      c=0,                              // category count
      p=0,                              // category page count
      cat_array = [],
      cat_date,
      vid_date;
...
function vertCentCats() {
  // Vertically center a CSS element greater than a given height
  (function(selector, max_height, center_margin) {
      var elems = $(selector);
      elems.each(function(index){
          if (elems.eq(index).height() > max_height) {
              elems.eq(index).css('margin', center_margin);
          }
      });
...
function setupRatingSubmit(){
    ...
    // var pollHost = 'https://web.archive.org/web/20160811090121/http://beta.polls.icarly.com';
...
function setNewVideo(vidId) {
  ...
  if (vidId && (vidId != -1)) {
    ...
    $('#videos ul.videos_playlist a[href*=(' + vidId + ']').addClass('active');
  }
}

function getDiscreteConfig() {
  if (discreteVid) {
    _config = ALL_CHANNELS[2].categories[2757].videoxml;
    var timestamp = _config.match(/.*?_(\d*)\.xml/);
    return '\/videoxml\/vid'+currVid.vid+'_'+timestamp[1]+'.xml';
  } else return NULL;
}

/*
// Make one of the tabs into a video category button - iparty
$(function(){
    var CATEGORY_ID = 2757,
        CHANNEL_ID = 2,
        TAB_CLASS = 'iparty',
        TAB_INDEX = 1,
        tabs = $('#videos_playlist_header a');

    tabs.eq(TAB_INDEX).addClass(TAB_CLASS).click(function(event){
        pickCategory(CATEGORY_ID, CHANNEL_ID);
...
// start playing next video on video end event
window.trackVideoEvent = function(action, label, id) {
    if (action == 'end') {
       findAndPlayNext(id);
    }
    $.trackEvent('video', action, label, 1);
}

// find next video in list
function findAndPlayNext(id){
   var thisLink = $('#videos_playlist_container a[href*="' + currVid.vid + '"]'),
       nextLink = thisLink.parent().next().find('a:first'); 
   
   if (nextLink.length) {
       nextLink.click();
   } else {
       thisLink.parents('ul').find('a:first').click();
   }
}

// get the video xml timestamp
function getTimeStamp(){
    return ALL_CHANNELS[ALL_CHANNELS.index_chan].videoxml.match(/.*?_(\d*)\.xml/)[1]   
}

function noAds() {
    return !!location.href.match( /ads=false/ )
}

function adFreeConfig( playerConfig ) {
    return playerConfig.match( /ad-free/ ) ? playerConfig :
           playerConfig.replace( 'videoxml/', 'videoxml/ad-free/' );
}

// bind video
$(function(){ 
    $('#videos_playlist_container a').click(function(e){
    
      var vid_id = this.href.match(/.*?video_(\d*)\D*/)[1],
          config = "/videoxml/vid-id-_-date-.xml",
          pageURL = this.href + ( noAds() ? '?ads=false' : '' );
           
      config = config.replace(/-id-/, vid_id)
      ...
      // change page title
      if (video_dictionary[vid_id]) {

          $('title:first').text(video_dictionary[vid_id].title + ' | Watch Free Videos and Clips | Official iCarly.com')
...
function overlayFunction(){
    $('#videos_playlist_container > div:not(".hide") a:first').click();
